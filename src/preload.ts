...
contextBridge.exposeInMainWorld('playwrightAPI', {
  ...
  searchByDateRange: (startDate: string, endDate: string, faturaTuru: '1' | '28' = '1') =>
    ipcRenderer.invoke('playwright:searchByDateRange', startDate, endDate, faturaTuru),
...
contextBridge.exposeInMainWorld('deeplinkAPI', {
  onParams: (callback: (params: { receteNo: string; barkodlar: string[]; kontrol: boolean }) => void) => {
